		...
		usi_app.load = function () {
			...
			try {
				...
				if (!usi_app.suppress_page) {
					if((usi_app.get_user_status("user").status == "guest" || usi_app.get_user_status("user").isActive == "false")) {
						// New Launch Logic 6/24
						usi_commons.log("[USI] --- NEW LAUNCH FLOW 6/24 ---")
						...
						if (((/.shutterstock.com\/image-generated|.shutterstock.com\/ai-image-generator|.shutterstock.com\/generate|.shutterstock.com\/search\?image_type=generated/i).test(location.href) || checkout_ai_eligible) && (language_key.includes("_english"))) {
							...
							usi_commons.log("[USI] 52759 - Precise Promotion | 10% Off Pre Cart - AI Generator Context");
							return usi_commons.load_view("NUqqoRwwRYghS7ZenwlRRQX", "52759", key);
						}
						...
						if (usi_app.is_creative_flow_page) {
							...
							usi_commons.log("[USI] 52759 - Precise Promotion | 10% Off Pre Cart - Creative Flow Context");
							return usi_commons.load_view("NUqqoRwwRYghS7ZenwlRRQX", "52759", key);
						}
						...
						if (usi_app.is_image_page || usi_app.is_image_in_cart) {
							...
							if (usi_app.messaging && usi_app.messaging.header1) {
								if (false && ((usi_app.is_cart_page)  || (usi_app.is_checkout_page && usi_cookies.value_exists('usi_prod_name_1')))) {
									usi_commons.log("[USI] 52761 - Precise Promotion | 10% Off Active Cart - Image Context");
									return usi_commons.load_view("7Iu6eQsC9zgaDfH3giUCiQP", "52761", key);
								} else {
									usi_commons.log("[USI] 52759 - Precise Promotion | 10% Off Pre Cart - Image Context");
									return usi_commons.load_view("NUqqoRwwRYghS7ZenwlRRQX", "52759", key);
								}
							}
							...
							if (usi_app.messaging && usi_app.messaging.header1) {
								if (false && ((usi_app.is_cart_page)  || (usi_app.is_checkout_page && usi_cookies.value_exists('usi_prod_name_1')))) {
									usi_commons.log("[USI] 52761 - Precise Promotion | 10% Off Active Cart - Video Context");
									return usi_commons.load_view("7Iu6eQsC9zgaDfH3giUCiQP", "52761", key);
								} else {
									usi_commons.log("[USI] 52759 - Precise Promotion | 10% Off Pre Cart - Video Context");
									return usi_commons.load_view("NUqqoRwwRYghS7ZenwlRRQX", "52759", key);
								}
							}
							...
							if (usi_app.messaging && usi_app.messaging.header1) {
								if (false && ((usi_app.is_cart_page)  || (usi_app.is_checkout_page && usi_cookies.value_exists('usi_prod_name_1')))) {
									usi_commons.log("[USI] 52761 - Precise Promotion | 10% Off Active Cart - Music Context");
									return usi_commons.load_view("7Iu6eQsC9zgaDfH3giUCiQP", "52761", key);
								} else {
									usi_commons.log("[USI] 52759 - Precise Promotion | 10% Off Pre Cart - Music Context");
									return usi_commons.load_view("NUqqoRwwRYghS7ZenwlRRQX", "52759", key);
								}
							}
						}
		...
		usi_app.get_language_key = function() {
			...
			try {
				if (usi_app.is_czech_language) {
					...
					language_key = (Math.random() < 0.33 ? language_key + '-a' : (Math.random() > 0.33 && Math.random() < 0.66 ? language_key + '-b' : language_key + '-c'));
					usi_commons.log("[usi] English Split Test - lang key changed to: " + language_key)
				}
			} catch (err) {
				usi_commons.report_error(err);
			}
